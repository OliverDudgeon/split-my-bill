...
var sumPricesByPerson = (prices) => {
  return range(prices[0].length).map((index) => sum(prices.map((item) => item[index])));
};
var poundFormatter = new Intl.NumberFormat("de-DE", {
  style: "currency",
  currency: "GBP"
});
...
var defaultNaturalNumberInputProps = {type: "number", min: 0, step: 1};
var calculatePriceSummaryFromFormData = (formData, receiptItems, numOfPeople) => {
  return receiptItems.map(([, price], itemIndex) => {
    const discount = Number(formData.get(`discount-${itemIndex}`));
    const shares = range(numOfPeople).map((personIndex) => Number(formData.get(`share-${itemIndex}-${personIndex}`)));
...
// build/dist/MainView.js
var CURRENCY_SYMBOLS = ["£", "$", "€"];
var divideReceipt = (source) => {
  const receiptItems = [];
  source.split("\n").map((line) => {
    for (const symbol of CURRENCY_SYMBOLS) {
      const [item, price] = line.split(symbol);
      price !== void 0 && receiptItems.push([item, parseFloat(price)]);
    }
  });
  return receiptItems;
};
